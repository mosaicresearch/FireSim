//Please don't alter the content of this file generated by FireSim
...
//Output switch
output :: Switch;
output[0] -> ToDump("output/faulty_accept.dump");
output[1] -> ToDump("output/faulty_reject.dump");
output[2] ->ToDump("output/faulty_drop.dump");
...
//Send faulty packet to correct dump
copy[0] -> faulty_queue :: SimpleQueue
-> faulty_unqueue :: Unqueue(LIMIT 1, ACTIVE false)
-> output;

//Do simulation with input traffic
copy[1] -> Script(TYPE PACKET, write bt.reset)
	-> MarkIPHeader(14)
	-> inputCounter;

//Send the packet (if faulty) to the correct dump file and get the next input packet going
	next :: Script(TYPE PACKET, write faulty_unqueue.active true)
 -> Script(TYPE PACKET, write faulty_unqueue.reset)
	-> Script(TYPE PACKET, write unqueue.reset)
 -> Discard;

//Feedback
ACCEPT_CHECK[0] -> ACCEPT_true -> Script(TYPE PACKET, write output.switch -1) -> next
ACCEPT_CHECK[1] -> ACCEPT_false -> Script(TYPE PACKET, write output.switch 0) -> next
REJECT[0] -> REJECT_true -> Script(TYPE PACKET, write output.switch -1) -> next
REJECT[1] -> REJECT_false -> Script(TYPE PACKET, write output.switch 1) -> next
DROP[0] -> DROP_true -> Script(TYPE PACKET, write output.switch -1) -> next
DROP[1] -> DROP_false -> Script(TYPE PACKET, write output.switch 2) -> next
